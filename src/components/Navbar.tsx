    ...
    <nav className="sticky top-0 z-50 border-b border-gray-200/80 bg-white/95 shadow-[0_1px_3px_rgba(0,0,0,0.04)] backdrop-blur-lg" aria-label="Main navigation">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 md:px-8">
        <Link href="/" className="shrink-0" aria-label="RankyPulse home">
          <Image src="/rankypulse-logo.svg" alt="RankyPulse" width={150} height={36} className="h-9 w-auto" priority />
        </Link>
        <div className="flex items-center gap-1 md:gap-2">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`rounded-xl px-4 py-2.5 text-sm font-medium transition-all ${pathname === link.href
